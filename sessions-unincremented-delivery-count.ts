...
async function main() {
  ...
  const createQueueResponse = await serviceBusAdministrationClient.createQueue(
    queueName,
    { enablePartitioning: true, requiresSession: true }
  );
  ...
  let receiver = await sbClient.acceptNextSession(queueName);
  await receiveMessages(receiver, numberOfMessagesToSend);
  await sbClient.close();
  console.log("Closed the client");

  sbClient = new ServiceBusClient(connectionString);
  receiver = await sbClient.acceptNextSession(queueName);
